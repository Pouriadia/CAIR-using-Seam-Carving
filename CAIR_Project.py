    ...
    for i in range(1, r):
        for j in range(c):
            if j == 0:
                ...
                min_cost = min(cost[i - 1, j - 1] + abs(img[i, j - 1] - img[i - 1, j]) + abs(img[i][j - 1] - img[i][j + 1]),
                               cost[i - 1, j + 1] + abs(img[i, j + 1] - img[i - 1, j]) + abs(img[i][j - 1] - img[i][j + 1]),
                               cost[i - 1, j] + abs(img[i][j - 1] - img[i][j + 1]))
...
def seam_carve(img, depth_map, saliency_map, num_seams, alpha, beta, gamma):
    for _ in range(num_seams):
        ...
        img = remove_seam_RGB(img, seam)
        depth_map = remove_seam_GS(depth_map, seam)
        saliency_map = remove_seam_GS(saliency_map, seam)
    return img
...
# Perform seam carving 2 in different ways
...
# Save the result
cv2.imwrite('Output\Baby_' + str(percentage) + '%' + '.png', output_img)